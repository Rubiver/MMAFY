...
기본 게임 서버 주소는 `ws://localhost:2567`입니다. 파일 변경 감시 없이 서버를 실행하려면 다음 명령을 사용합니다.
...
Vite가 출력한 주소로 접속합니다. 기본 개발 주소는 보통 `http://localhost:5173`이며, 클라이언트는 기본적으로 `ws://localhost:2567` 서버에 연결합니다.

서버 주소를 바꿨다면 클라이언트에도 같은 주소를 전달합니다.
