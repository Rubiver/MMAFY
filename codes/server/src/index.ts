...
setInterval(() => { for (const [roomCode, entry] of rooms) { const now = Date.now(); entry.room.pruneDisconnected(now); if (entry.room.advance(now)) broadcastState(roomCode); if (entry.room.snapshot().players.length === 0) rooms.delete(roomCode); } }, 250).unref();
...
function parseMessage(raw: string): ClientMessage {
  ...
  if (value.type === "ENVIRONMENT" && ["SABOTAGE", "REPAIR_START", "REPAIR_COMPLETE", "REPAIR_CANCEL", "VENT", "TASK", "DOOR_TOGGLE", "DOOR_LOCK", "CCTV_OPEN", "CCTV_CLOSE", "COMM_SABOTAGE", "COMM_REPAIR"].includes(String(value.action)) && (value.deviceId === undefined || value.deviceId === "generator-a" || value.deviceId === "generator-b") && (value.puzzle === undefined || Array.isArray(value.puzzle) && value.puzzle.every((item) => typeof item === "string") && value.puzzle.length <= 8)) return value as ClientMessage;
