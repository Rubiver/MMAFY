  ...
  socket.on("message", (raw) => {
    try {
      ...
      if (message.type === "CREATE_ROOM") {
        ...
        else if (message.type === "REPORT") { if (entry.environment.snapshot().criticalSabotageEndsAt !== undefined) throw new RoomError("INVALID_MESSAGE", "긴급 과부하를 먼저 복구해야 합니다."); room.report(session.playerId, message.bodyId, now); broadcastState(session.roomCode); }
        else if (message.type === "CALL_MEETING") { if (entry.environment.snapshot().criticalSabotageEndsAt !== undefined) throw new RoomError("INVALID_MESSAGE", "긴급 과부하 중에는 회의를 열 수 없습니다."); room.callMeeting(session.playerId, now); broadcastState(session.roomCode); }
