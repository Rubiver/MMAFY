...
const devices: InteractableState[] = [
  ...
  { id: "data-sorter", name: "자료 정렬 단말", type: "DATA_SORTER", position: DATA_SORTER_POSITION, interactionRange: 2.4, currentState: "READY" },
  { id: "coolant-mixer", name: "냉각수 배합기", type: "COOLANT_MIXER", position: COOLANT_MIXER_POSITION, interactionRange: 2.4, currentState: "READY" },
...
function Device({ device }: { device: InteractableState }) {
  const colors = { GENERATOR: "#f4b942", DOOR: "#4ca7e8", LADDER: "#81c784", VENT: "#a78bfa", BELL: "#facc15", TASK_PANEL: "#22d3ee", SECURITY_CARD: "#38bdf8", DATA_SORTER: "#c084fc", COOLANT_MIXER: "#2dd4bf", COOP_TASK: "#2dd4bf", CCTV: "#67e8f9", COMMUNICATIONS: "#f472b6", CARGO_PICKUP: "#fb923c", CARGO_DELIVERY: "#34d399" };
  ...
  const criticalRepaired = useGameStore((state) => state.environment?.criticalRepairedGeneratorIds) ?? [];
  ...
  const usable = nearbyDeviceId === device.id && (device.type === "BELL" || (device.type === "TASK_PANEL" || device.type === "SECURITY_CARD") && role === "SURVIVOR" || device.type === "DATA_SORTER" && role === "SURVIVOR" && !dataSortCompleted.includes(playerId ?? "") || device.type === "COOLANT_MIXER" && role === "SURVIVOR" && !coolantCompleted.includes(playerId ?? "") || device.type === "COOP_TASK" && role === "SURVIVOR" && !blackout && !cooperativeCompleted || device.type === "CARGO_PICKUP" && role === "SURVIVOR" && !cargoCarriers.includes(playerId ?? "") && !cargoCompleted.includes(playerId ?? "") || device.type === "CARGO_DELIVERY" && role === "SURVIVOR" && cargoCarriers.includes(playerId ?? "") || device.type === "CCTV" && role === "SURVIVOR" && cctvOnline || device.type === "COMMUNICATIONS" && role === "SURVIVOR" && !communicationsOnline || device.type === "DOOR" && (role === "MAFIA" && doorState === "CLOSED" || role === "SURVIVOR" && doorState !== "LOCKED") || device.type === "VENT" && role === "MAFIA" || device.type === "GENERATOR" && role === "SURVIVOR" && (blackout || criticalActive && !criticalRepaired.includes(device.id as GeneratorId)));
...
function GeneratorBlackoutOutline() {
  ...
  const role = useGameStore((state) => state.role);
  if ((!blackout && !criticalActive) || role !== "SURVIVOR") return null;
...
function PlayerController() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (device.type === "DATA_SORTER") { if (state.role !== "SURVIVOR") setInteractionMessage("자료 정렬은 시민만 수행할 수 있습니다."); else if (state.environment?.dataSortCompletedIds.includes(state.playerId ?? "")) setInteractionMessage("이번 판의 자료 정렬을 이미 완료했습니다."); else { document.exitPointerLock(); setDataSortPanelOpen(true); } return; }
      if (device.type === "COOLANT_MIXER") { if (state.role !== "SURVIVOR") setInteractionMessage("냉각수 배합은 시민만 수행할 수 있습니다."); else if (state.environment?.coolantCompletedIds.includes(state.playerId ?? "")) setInteractionMessage("이번 판의 냉각수 배합을 이미 완료했습니다."); else { document.exitPointerLock(); setCoolantPanelOpen(true); } return; }
      ...
      if (device.type === "GENERATOR" && state.environment?.criticalSabotageEndsAt !== undefined) { if (state.role !== "SURVIVOR") setInteractionMessage("긴급 과부하는 시민만 복구할 수 있습니다."); else if (state.environment.criticalRepairedGeneratorIds.includes(device.id as GeneratorId)) setInteractionMessage(`${device.name} 긴급 초기화를 이미 완료했습니다.`); else { setInteractionMessage(`${device.name} 긴급 초기화를 요청합니다.`); getActiveGameClient()?.environment("CRITICAL_REPAIR", device.id as GeneratorId); } return; }
  ...
  useFrame((_state, delta) => {
    ...
    const positionError = authoritativePosition ? Math.hypot(localPosition.current.x - authoritativePosition.x, localPosition.current.z - authoritativePosition.z) : 0;
    const stopped = !input.forward && !input.backward && !input.left && !input.right;
    if (authoritativePosition && (positionError > (stopped ? 0.75 : 10) || locallyBlocked)) {
...
function GamePointerLockControls() {
  ...
  const overlayOpen = taskPanelOpen || securityCardPanelOpen || dataSortPanelOpen || coolantPanelOpen || cctvOpen;
  useEffect(() => {
    if (!overlayOpen) return;
    /** 임무 화면 중 늦게 완료된 잠금 요청까지 즉시 해제한다. */
    const releasePointer = () => { if (document.pointerLockElement) document.exitPointerLock(); };
    releasePointer();
    document.addEventListener("pointerlockchange", releasePointer);
    return () => document.removeEventListener("pointerlockchange", releasePointer);
  }, [overlayOpen]);
  return <PointerLockControls enabled={lifeState === "ALIVE" && !overlayOpen} />;
}
