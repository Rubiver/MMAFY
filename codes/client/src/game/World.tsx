...
function Device({ device }: { device: InteractableState }) {
  ...
  const usable = nearbyDeviceId === device.id && (device.type === "BELL" || (device.type === "TASK_PANEL" || device.type === "SECURITY_CARD") && role === "SURVIVOR" || device.type === "COOP_TASK" && role === "SURVIVOR" && !blackout && !cooperativeCompleted || device.type === "CARGO_PICKUP" && role === "SURVIVOR" && !cargoCarriers.includes(playerId ?? "") && !cargoCompleted.includes(playerId ?? "") || device.type === "CARGO_DELIVERY" && role === "SURVIVOR" && cargoCarriers.includes(playerId ?? "") || device.type === "CCTV" && role === "SURVIVOR" && cctvOnline || device.type === "COMMUNICATIONS" && role === "SURVIVOR" && !communicationsOnline || device.type === "DOOR" && (role === "MAFIA" && doorState === "CLOSED" || role === "SURVIVOR" && doorState !== "LOCKED") || device.type === "VENT" && role === "MAFIA" || device.type === "GENERATOR" && role === "SURVIVOR" && (blackout || criticalActive && !criticalRepaired.includes(device.id as GeneratorId)));
