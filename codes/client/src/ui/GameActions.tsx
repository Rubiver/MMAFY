 ...
 */
type SabotageTarget = GeneratorId | "communications" | "critical";
function SabotageMap({ held, blackout, onSabotage }: { held: boolean; blackout: boolean; onSabotage: (target: SabotageTarget) => void }) {
  const criticalActive = useGameStore((state) => state.environment?.criticalSabotageEndsAt !== undefined);
  if (!held) return null;
  const points: { id: SabotageTarget; name: string; location: string; position: string }[] = [{ id: "generator-a", name: "발전기 A", location: "서쪽 전력실", position: "left-[15%] top-[62%]" }, { id: "generator-b", name: "발전기 B", location: "동쪽 전력실", position: "right-[12%] top-[28%]" }, { id: "communications", name: "통신 장치", location: "동쪽 통신실", position: "right-[26%] top-[67%]" }, { id: "critical", name: "핵심 과부하", location: "60초 긴급 임무", position: "left-1/2 top-[28%]" }];
  return <section className="fixed inset-0 z-30 flex items-center justify-center bg-slate-950/45 p-3 sm:p-6" aria-label="사보타지 시설 지도"><div className="w-full max-w-2xl rounded-3xl border border-rose-300/40 bg-slate-950/95 p-4 shadow-2xl shadow-black/60 backdrop-blur-md sm:p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-bold tracking-[0.16em] text-rose-300 sm:text-xs sm:tracking-[0.2em]">CAPS 유지 중 · 마피아 전용</p><h2 className="mt-1 text-xl font-bold text-white sm:text-2xl">시설 원격 방해공작</h2></div><span className="shrink-0 whitespace-nowrap rounded-full border border-rose-200/30 bg-rose-500/15 px-3 py-1 text-[10px] font-semibold text-rose-100 sm:text-xs">Caps를 놓으면 닫힘</span></div><p className="mt-2 text-sm text-slate-300">핵심 과부하는 60초 안에 양쪽 발전기를 복구하지 못하면 마피아가 승리합니다.</p><div className="relative mt-5 h-64 overflow-hidden rounded-2xl border border-slate-700 bg-[radial-gradient(circle_at_70%_35%,#23384d_0,transparent_20%),linear-gradient(135deg,#101c2a,#071018)] sm:h-80"><div className="absolute inset-x-10 top-1/2 h-px bg-cyan-200/15" /><div className="absolute bottom-7 left-1/2 top-7 w-px bg-cyan-200/15" /><span className="absolute bottom-4 left-4 text-xs text-slate-500">서쪽 전력실</span><span className="absolute right-4 top-4 text-xs text-slate-500">동쪽 전력실</span>{points.map((point) => <button key={point.id} type="button" disabled={blackout || criticalActive} onClick={() => onSabotage(point.id)} className={`absolute ${point.position} -translate-x-1/2 -translate-y-1/2 rounded-xl border border-rose-200/60 bg-rose-600 px-3 py-2 text-left text-white shadow-lg shadow-rose-950/50 transition hover:scale-105 hover:bg-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-200 disabled:cursor-not-allowed disabled:border-slate-600 disabled:bg-slate-800 disabled:text-slate-400`}><span className="block text-base leading-none" aria-hidden="true">⚡</span><b className="mt-1 block text-xs">{point.name}</b><small className="block text-[10px] text-rose-100">{point.location}</small></button>)}</div></div></section>;
}
...
function CooperativeTaskIndicator() {
  ...
  if (!playerId || !participants.includes(playerId)) return <CriticalSabotageIndicator />;
  const ready = participants.length >= 2;
  const seconds = Math.max(0, Math.ceil((1 - progress) * COOPERATIVE_TASK_DURATION_MS / 1000));
  return <><CriticalSabotageIndicator /><div className="pointer-events-none fixed left-1/2 top-1/2 z-20 w-[min(21rem,calc(100vw-2rem))] -translate-x-1/2 translate-y-8 rounded-xl border border-teal-200/50 bg-slate-950/90 p-3 text-teal-50 shadow-2xl backdrop-blur-md" aria-label={`협동 임무 참가 ${participants.length}명`}><div className="flex items-center justify-between gap-3 text-xs font-bold"><span>{ready ? "교량 동기화 진행 중" : "동료 시민 대기 중"}</span><span>{participants.length} / 2명{ready ? ` · ${seconds}초` : ""}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-800"><div className={`h-full rounded-full transition-[width] duration-200 ${ready ? "bg-teal-300" : "bg-amber-300"}`} style={{ width: `${ready ? progress * 100 : 8}%` }} /></div><p className="mt-2 text-[11px] text-slate-300">두 시민 모두 [E]를 유지해야 합니다. 이탈·사망·정전 시 처음부터 다시 시작합니다.</p></div></>;
}

/** 제한 시간 긴급 과부하와 발전기별 복구 상태를 모든 참가자에게 표시한다. */
function CriticalSabotageIndicator() {
  const endsAt = useGameStore((state) => state.environment?.criticalSabotageEndsAt);
  const repaired = useGameStore((state) => state.environment?.criticalRepairedGeneratorIds) ?? [];
  const [now, setNow] = useState(Date.now());
  useEffect(() => { const timer = window.setInterval(() => setNow(Date.now()), 100); return () => window.clearInterval(timer); }, []);
  if (endsAt === undefined) return null;
  const remaining = Math.max(0, Math.ceil((endsAt - now) / 1000));
  const time = `${Math.floor(remaining / 60)}:${String(remaining % 60).padStart(2, "0")}`;
  return <aside className="pointer-events-none fixed bottom-28 right-3 z-20 w-[calc(100vw-1.5rem)] overflow-hidden rounded-2xl border border-rose-200/70 bg-slate-950/95 p-4 text-white shadow-2xl shadow-rose-950/70 backdrop-blur-md sm:bottom-auto sm:right-6 sm:top-6 sm:w-80" aria-label={`핵심 전력 과부하 남은 시간 ${time}`}><div className="absolute inset-x-0 top-0 h-1 animate-pulse bg-rose-500" /><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black tracking-[0.18em] text-rose-300">긴급 사보타지</p><h2 className="mt-1 text-lg font-black">핵심 전력 과부하</h2></div><strong className={`font-mono text-3xl tabular-nums ${remaining <= 15 ? "animate-pulse text-rose-300" : "text-amber-200"}`}>{time}</strong></div><div className="mt-3 grid grid-cols-2 gap-2">{(["generator-a", "generator-b"] as const).map((id, index) => { const done = repaired.includes(id); return <span key={id} className={`rounded-xl border px-3 py-2 text-xs font-bold ${done ? "border-emerald-300/50 bg-emerald-400/15 text-emerald-200" : "border-rose-300/40 bg-rose-500/15 text-rose-100"}`}>발전기 {index === 0 ? "A" : "B"} · {done ? "복구 완료" : "초기화 필요"}</span>; })}</div><p className="mt-3 text-xs leading-5 text-slate-200">시민은 양쪽 발전기에서 [E]를 눌러 복구하세요. 시간이 끝나면 마피아가 즉시 승리합니다.</p></aside>;
}
...
function SecurityCardPanel() {
  ...
  return <section className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/80 p-4" aria-label="보안 카드 인증"><div className="w-full max-w-lg rounded-3xl border border-sky-200/40 bg-slate-900 p-5 shadow-2xl shadow-sky-950/50 sm:p-7"><p className="text-xs font-bold tracking-[0.16em] text-sky-300">공통 임무 · 보안 카드</p><h2 className="mt-2 text-2xl font-bold text-white">표시된 방향으로 카드를 인증하세요</h2><p className="mt-2 text-sm leading-6 text-slate-300">인증 패턴은 <b className="text-sky-200">왼쪽 → 위 → 오른쪽 → 아래</b>입니다. 서버가 순서·거리·중복 완료를 확인합니다.</p><div className="mt-5 flex h-16 items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-950/80">{[0, 1, 2, 3].map((index) => <span key={index} className="flex h-10 w-14 items-center justify-center rounded-xl bg-slate-800 text-xl font-bold text-sky-200">{directions.find((item) => item.id === pattern[index])?.symbol ?? "?"}</span>)}</div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{directions.map((direction) => <button key={direction.id} type="button" onClick={() => choose(direction.id)} className="min-h-20 rounded-2xl border border-sky-200/30 bg-sky-500/15 px-3 py-3 text-sky-50 transition hover:bg-sky-500/25 focus:outline-none focus:ring-2 focus:ring-sky-200"><span className="pointer-events-none block text-2xl" aria-hidden="true">{direction.symbol}</span><b className="pointer-events-none mt-1 block text-sm">{direction.name}</b></button>)}</div><button type="button" onClick={close} className="mt-4 w-full rounded-xl bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 hover:bg-slate-700">취소</button></div></section>;
}

/** 시민이 뒤섞인 자료 묶음을 번호순으로 정렬하는 화면이다. */
function DataSortPanel() {
  const open = useGameStore((state) => state.dataSortPanelOpen);
  const setOpen = useGameStore((state) => state.setDataSortPanelOpen);
  const setMessage = useGameStore((state) => state.setInteractionMessage);
  const [order, setOrder] = useState<string[]>([]);
  if (!open) return null;
  const close = () => { setOrder([]); setOpen(false); restoreGamePointerLock(); };
  const choose = (value: string) => { const next = [...order, value]; if (next.length < 4) { setOrder(next); return; } getActiveGameClient()?.environment("DATA_SORT_TASK", undefined, next); close(); setMessage("자료 정렬 결과를 서버에 확인합니다."); };
  return <section className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/80 p-3 sm:p-6" aria-label="자료 정렬 임무"><div className="w-full max-w-lg rounded-3xl border border-fuchsia-200/40 bg-slate-900 p-5 shadow-2xl shadow-fuchsia-950/40 sm:p-7"><p className="text-xs font-bold tracking-[0.16em] text-fuchsia-300">공통 임무 · 자료 정렬</p><h2 className="mt-2 text-xl font-bold text-white sm:text-2xl">자료 묶음을 작은 번호부터 선택하세요</h2><p className="mt-2 text-sm leading-6 text-slate-300">정렬 순서는 <b className="text-fuchsia-200">2 → 4 → 7 → 9</b>입니다. 서버가 순서와 중복 완료를 확인합니다.</p><div className="mt-5 flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-950/80">{[0, 1, 2, 3].map((index) => <span key={index} className="flex h-10 w-14 items-center justify-center rounded-xl bg-fuchsia-400/15 text-lg font-black text-fuchsia-100">{order[index] ?? "?"}</span>)}</div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{["7", "2", "9", "4"].map((value) => <button key={value} type="button" disabled={order.includes(value)} onClick={() => choose(value)} className="min-h-16 rounded-2xl border border-fuchsia-200/30 bg-fuchsia-500/15 text-xl font-black text-fuchsia-50 transition hover:bg-fuchsia-500/25 disabled:cursor-not-allowed disabled:opacity-35">{value}</button>)}</div><button type="button" onClick={close} className="mt-4 w-full rounded-xl bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 hover:bg-slate-700">취소</button></div></section>;
}

/** 시민이 세 냉각수 계통을 목표 비율로 맞추는 화면이다. */
function CoolantPanel() {
  const open = useGameStore((state) => state.coolantPanelOpen);
  const setOpen = useGameStore((state) => state.setCoolantPanelOpen);
  const setMessage = useGameStore((state) => state.setInteractionMessage);
  const [ratios, setRatios] = useState([0, 0, 0]);
  if (!open) return null;
  const labels = ["청록 계통", "파랑 계통", "보라 계통"];
  const targets = [30, 50, 20];
  const close = () => { setRatios([0, 0, 0]); setOpen(false); restoreGamePointerLock(); };
  const submit = () => { getActiveGameClient()?.environment("COOLANT_TASK", undefined, ratios.map(String)); close(); setMessage("냉각수 배합 결과를 서버에 확인합니다."); };
  return <section className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/80 p-3 sm:p-6" aria-label="냉각수 배합 임무"><div className="max-h-[calc(100vh-1.5rem)] w-full max-w-xl overflow-y-auto rounded-3xl border border-teal-200/40 bg-slate-900 p-5 shadow-2xl shadow-teal-950/40 sm:p-7"><p className="text-xs font-bold tracking-[0.16em] text-teal-300">공통 임무 · 냉각수 배합</p><h2 className="mt-2 text-xl font-bold text-white sm:text-2xl">세 계통을 목표 비율로 맞추세요</h2><p className="mt-2 text-sm leading-6 text-slate-300">목표는 <b className="text-teal-200">30% · 50% · 20%</b>입니다. 각 조절 막대를 10% 단위로 움직이세요.</p><div className="mt-5 space-y-4">{labels.map((label, index) => <label key={label} className="block rounded-2xl border border-teal-200/20 bg-teal-400/10 p-4"><span className="flex items-center justify-between text-sm font-bold text-teal-50"><span>{label}</span><span>{ratios[index]}% / {targets[index]}%</span></span><input aria-label={label} className="mt-3 w-full accent-teal-300" type="range" min="0" max="100" step="10" value={ratios[index]} onChange={(event) => setRatios((current) => current.map((value, itemIndex) => itemIndex === index ? Number(event.target.value) : value))} /></label>)}</div><button type="button" onClick={submit} className="mt-5 w-full rounded-xl bg-teal-400 px-4 py-3 text-sm font-black text-slate-950 hover:bg-teal-300">배합 제출</button><button type="button" onClick={close} className="mt-3 w-full rounded-xl bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 hover:bg-slate-700">취소</button></div></section>;
}
...
export function GameActions() {
  ...
  const dataSortPanelOpen = useGameStore((state) => state.dataSortPanelOpen);
  const setDataSortPanelOpen = useGameStore((state) => state.setDataSortPanelOpen);
  const coolantPanelOpen = useGameStore((state) => state.coolantPanelOpen);
  const setCoolantPanelOpen = useGameStore((state) => state.setCoolantPanelOpen);
  ...
  }, [coolantPanelOpen, cctvOpen, dataSortPanelOpen, role, securityCardPanelOpen, setCoolantPanelOpen, setCctvOpen, setDataSortPanelOpen, setSecurityCardPanelOpen, setTaskPanelOpen, taskPanelOpen]);
  ...
  const sabotage = (target: SabotageTarget) => { if (!canAct) return; if (target === "critical") getActiveGameClient()?.environment("CRITICAL_SABOTAGE"); else if (target === "communications") getActiveGameClient()?.environment("COMM_SABOTAGE"); else getActiveGameClient()?.environment("SABOTAGE", target); };
